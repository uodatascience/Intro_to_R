...
-   Indexing (Jonny) \*\* stacking indexing \[\]\[\]\[\] \*\*
    dplyr::select and variations like contains, one\_of \*\* use of drop
    (preserving vs simplifying) \*\* sampling \*\* logical indexing &
    booleans (incl. & vs &&) & converting w/ which \*\* subsetting with
    assignment (x\[i\] &lt;- 1) \*\* creating with subsetting (match,
    using one vector to pull rows from a df, doing a gradebook w/
    commnets), and destroying (unique, dplyr::distinct)

-   Manipulating \*\* Wide v Long data as intro \*\* piping
    syntax (incl. when to use/not use) \*\* reshaping - gather/melt &
    spread/dcast
    (<https://www.r-bloggers.com/how-to-reshape-data-in-r-tidyr-vs-reshape2/>)
    \*\* reordering (incl. reordering factor levels, talking abt
    factors) \*\* separate & colsplit \*\* combining dataframes (if this
    makes sense to do/there's time to do it) (see relational data
    in r4ds)

-   Annoyances \*\* dealing with `NaN`s \*\* typing your data (mainly to
    cover factors, ordered v. unordered)
