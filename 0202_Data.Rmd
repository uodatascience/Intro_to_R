...
The most important remaining part of this syntax is the columns that you want to *gather*. If you don't name any columns, it will select all of them. Alternatively, you can tell it not to select one of the columns. In our example, we want to select all of the columns except `pid`, which is the participant id. So we would write some code like this:
```{r lengthening data with gather 1}
sample_b5_data_wide %>%
  gather(variable, score, -pid)
```

Okay, so this brings us to our first issue when trying to lengthen a dataset. The key column (what we called variable) contains the trait that was measured and the time point. We want these to be in separate columns, so that we can select certain traits or time points, or model slopes across time points.
