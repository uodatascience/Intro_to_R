...
Well, basically, separate looks for something called *regular expressions*. We'll get into more detail about regular expressions later on, but one regular expression is '_' for separating things. So it recognized that every string in the variable column contained a "_". Then, it put the character string on the left hand side of the "_" into the first column name we gave it (trait) and put the character string on the right hand side of the "_" into the second column name we gave it (time). If we wanted to be more explicit, we could specify the `sep=` argument like so:
